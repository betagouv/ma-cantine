...
def calcul_indicateurs_secteurs(tds, year, suffix=''):
    ...
    # TODO : les inclure sous une nouvelle catégorie
    ...
    for cat in utils.SECTORS.keys():
        ...
        if cat in df_processed_sectors.columns:
            ...
            sectors[year][cat][f'Taux des achats EGAlim{suffix}'] = (df_cat["teledeclaration.value_bio_ht"].sum() + df_cat["teledeclaration.value_externality_performance_ht"].sum() + df_cat["teledeclaration.value_sustainable_ht"].sum() + df_cat["teledeclaration.value_egalim_others_ht"].sum()) / df_cat["teledeclaration.value_total_ht"].sum()
    ...
    # Suppression des colonnes après leur utilisation pour les calculs de ratio
