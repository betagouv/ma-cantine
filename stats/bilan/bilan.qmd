...
if not cache:
    ...
    for year in utils.CAMPAGNES.keys():
        ...
        utils.TUNNEL_DATA_ENG[year]['Après application stratégie Je Ne Sais Pas'] = len(tds[f'Campagne {year}'])
...
if not cache:
    utils.TUNNEL_DATA_ENG['2023']['Après séléction des assidus'] = len(tds_commun['2023'])
...
if not cache:
    utils.TUNNEL_DATA_ENG['2023']['Après filtrage des assidus'] = len(tds_commun['2023'])
...
if not cache:
    ...
    with plt.style.context("fivethirtyeight"):
        ...
        plt.title("Traitement des télédéclarations", loc="right", fontsize=25, fontweight="bold")
        plt.show()
...
### Focus par type de management (concédé ou direct)
#### Chiffre généraux
```{python Nombre de sites de restauration concernés par la télédéclaration}
# | echo: false
# | output: asis
tds['gestion_directe_2023'] = tds['Campagne 2023'][tds['Campagne 2023']['canteen.management_type'] == 'direct']
tds['gestion_conceded_2023'] = tds['Campagne 2023'][tds['Campagne 2023']['canteen.management_type'] == 'conceded']
indic_divers, indic_appro = utils.calcul_indicateur(tds, years=["gestion_directe_2023", "gestion_conceded_2023"])

indic_divers = indic_divers.rename(columns={'gestion_directe_2023': 'Campagne 2023 - Cantines en gestion directe', 'gestion_conceded_2023': 'Campagne 2023 - Cantines en gestion concédée'})
indic_appro = indic_appro.rename(columns={'gestion_directe_2023': 'Campagne 2023 - Cantines en gestion directe', 'gestion_conceded_2023': 'Campagne 2023 - Cantines en gestion concédée'})
utils.display_indicateurs(indic_divers[['Campagne 2023 - Cantines en gestion directe', 'Campagne 2023 - Cantines en gestion concédée']])
```
...
utils.display_indicateurs(indic_appro[['Campagne 2023 - Cantines en gestion directe', 'Campagne 2023 - Cantines en gestion concédée']])
```
...
col_order = [
    ...
    "Nombre de TD ayant déclaré pour la famille Viande/Volaille 🥩",
    "Nombre de TD ayant déclaré la valeur 0€ pour la famille Viande/Volaille 🥩",
    "Montant des achats alimentaires totaux pour la famille Viande/Volaille 🥩",
    "Taux d'achat alimentaires de la famille Viande/Volaille 🥩",
    "Montant d'achat alimentaires Viande/Volaille 🥩",
    "Taux d'achat alimentaires Egalim au sein de la famille Viande/Volaille 🥩",
    "Montant d'achat alimentaires Egalim Viande/Volaille 🥩",
    "Taux d'achat alimentaires origine France 🇫🇷 au sein de la famille Viande/Volaille 🥩",
    "Montant d'achat alimentaires origine France 🇫🇷 Viande/Volaille 🥩",
    "Nombre de TD ayant déclaré pour la famille Poissons/Produits de la mer 🐟",
    "Nombre de TD ayant déclaré la valeur 0€ pour la famille Poissons/Produits de la mer 🐟",
    "Montant des achats alimentaires totaux pour la famille Poissons/Produits de la mer 🐟",
    "Taux d'achat alimentaires de la famille Poissons/Produits de la mer 🐟",
    "Taux d'achat alimentaires Egalim au sein de la famille Poissons/Produits de la mer 🐟",
    "Montant d'achat alimentaires Poissons/Produits de la mer 🐟",
    "Montant d'achat alimentaires Egalim Poissons/Produits de la mer 🐟"
]
...
ax.set_ylabel('Nombre de télédéclarations',)
ax.set_title('Distribution des télédéclarants en fonction de la taille des cantines \n(basé sur le nombre de repas servis par jour)')
...
# Evolution des télédéclaration lors de la campagne 2023
```{python}
...
## Nombre de télédéclarations ayant rempli les objectifs bio et Egalim
...
print(f'{len(df)} télédéclarants remplissent les objectifs Bio\n')
df = tds['Campagne 2023'].copy()
df = df[df['Distribution des taux Egalim'] >= 50]
print(f'{len(df)} télédéclarants remplissent les objectifs Egalim\n')
df = tds['Campagne 2023'].copy()
df = df[df['Distribution des taux Bio'] >= 20]
df = df[df['Distribution des taux Egalim'] >= 50]
print(f'{len(df)} télédéclarants remplissent les objectifs Bio et Egalim')
```