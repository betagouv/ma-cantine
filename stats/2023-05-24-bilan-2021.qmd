...
* Les données sont issus d'une extractation de la base de données de l'application Ma-Cantine, en filtrant sur :
    ...
    * Les canteen ID qui sont renseignés
...
#     # if type(td[col]) == "object":
#     td[col] = td[col].astype("string")
# profile = ProfileReport(td, title="Profiling Report")
# profile.to_file("your_report.html")
# profile.to_notebook_iframe()
```

```{python Verificaiton des valeurs en recréant l'erreur}
...
indicateurs["Nombre de repas moyens par jour pour les cantines sur place"] = int(
    cuisines_sur_place["daily_meal_count"].mean()
)
indicateurs["Nombre de repas moyens par jour pour les cantines centrales"] = int(
    cuisines_centrales["daily_meal_count"].mean()
)
...
] = f"{100 * len(td[td.management_type == 'direct']) / len(td):.2f} %"
indicateurs[
    "Répartition du nombre de cantines en gestion concédée"
] = f"{100 * len(td[td.management_type == 'conceded']) / len(td):.2f} %"
indicateurs[
    "Répartition du nombre de cantines en gestion non renseignée"
] = f"{100 * len(td[~td.management_type.isin(['direct', 'conceded'])]) / len(td):.2f} %"
```
...
print(
    f"Le taux de présence de la donnée repas par an est de {100 * (1 - td['yearly_meal_count'].isna().sum() / len(td)):.2f} %"
)
```
...
La communication avait été lancée mi-juillet. Cependant la fonctionnalité existait déjà.

* Que fait on des TD hors cadre d'une campagne ?
* Existe-il des cantines de test dans la campagne ? (apparement non)

* D'ou sortent les 300 TD d'avant le lancement de la campagne ? => Bizdev ? => A vérifier  

A faire : 
* Verifier le taux de presence de la donnée repas par an
* Relancer en supprimant les 8 cantines