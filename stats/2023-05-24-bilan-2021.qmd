...
# Filtrage
```{python Filtrage des données}
td_date = td[td['creation_date'] >= '2022-01-01']
print(len(td_date))
```
...
```{python}
display(td['test' in td['canteen_name']].production_type)
```

```{python}
```

## Attention : filtrer les cantines test