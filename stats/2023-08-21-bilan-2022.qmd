...
for year in campagnes.keys():
    ...
    satellites_sirets_raw = df.apply(lambda x: [item["siret"] for item in x]).values.tolist()
...
print(
    f"Il y a {len(sortants)} teledeclarant n'ayant pas renouvellé (ayant déclaré sur la campagne 2022 mais pas celle de 2023)"
)
```

Analysons les profils entrants et sortants.  
**En cours**
