<template>
  ...
  >
    ...
    <div class="grey--text text--darken-2" :style="$vuetify.display.smAndDown ? '' : 'height: 95px;'">
      <v-card-text v-if="diagnostic" class="py-1 fill-height d-flex flex-column">
        <span>En {{ year }} :</span>
        <v-row v-if="hasPercentages" class="ma-0">
          <p v-if="bioPercent" class="ma-0 mr-3">
            <span class="font-weight-black mr-1">{{ bioPercent }} %</span>
            <span>bio</span>
          </p>
          <p v-if="sustainablePercent" class="ma-0">
