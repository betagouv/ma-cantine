<template>
  <div>
    ...
    <div class="d-flex align-center mt-4 pl-0 pl-md-6">
      ...
      <v-btn v-if="hasActiveFilter" text color="primary" small @click="clearFilters">
    ...
    <div v-else-if="publishedCanteenCount === 0" class="d-flex flex-column align-center py-10">
      ...
      <v-btn v-if="hasActiveFilter" color="primary" text class="text-decoration-underline" @click="clearFilters">
        Désactiver tous les filtres
      </v-btn>
    </div>
    <div v-else>
      <p v-if="resultsCountText" class="mt-3 mb-n4 text-body-2 grey--text">
        {{ resultsCountText }}
      </p>
      <v-row class="my-2" align="end">
        ...
        <v-col id="ordering" cols="12" sm="3" class="d-flex align-end">
          <DsfrSelect
            v-model="orderBy"
            :items="orderOptions"
            label-classes="body-2 text-left mb-2"
            hide-details
            label="Trier par"
          />
          <v-btn
