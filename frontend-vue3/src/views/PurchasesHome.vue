    ...
    <div class="d-flex">
      <div>
        ...
        <DsfrTable
          ...
          :headers="headers"
          :rows="[
            ['15 août 2023', 'Fraises', 'FRUITS_ET_LEGUMES', 'LABEL_ROUGE', 'undefined', '877', 'false'],
            ['3 août 2023', 'Tofu', 'AUTRES', 'SHORT_DISTRIBUTION', 'undefined', '34', 'false'],
            ['6 juin 2023', 'Bananes', 'FRUITS_ET_LEGUMES', 'AOCAOP', 'undefined', '65', 'false'],
            ['4 oct. 2022', 'Oranges', 'FRUITS_ET_LEGUMES', 'IGP', 'undefined', '123', 'false'],
          ]"
          :no-caption="false"
          :pagination="true"
          :defaultCurrentPage="options.page"
          :defaultOptionSelected="limit"
          v-if="dsfrRows"
        ></DsfrTable>
        TABLE

        <DsfrTable title="Pets" :headers="['Name', 'Kind']" :rows="pets" />
        ...
        <!--<v-row v-if="hasCanteens" align="center" class="px-3">
          ...
          <v-btn text color="primary" :to="{ name: 'PurchasesSummary' }" class="px-0 px-md-2 my-3">
      ...
      <!-- <v-card class="dsfr d-flex flex-column justify-center" :to="{ name: 'PurchasesSummary' }" min-width="300px">
...
export default {
  ...
  data() {
    return {
      ...
      headers: ["Date", "Produit", "Famille", "Caratéristiques", "Cantine", "Prix", ""],
      oldHeaders: [
        {
          text: "Date",
          align: "start",
  ...
  computed: {
    ...
    processedVisiblePurchases() {
      if (!this.$store.state.userCanteenPreviews || !this.visiblePurchases) return null
    ...
    dsfrRows() {
      const dsfrRows = []
      if (!this.processedVisiblePurchases) return dsfrRows
      return this.processedVisiblePurchases.map((x) => [
        x.date,
        x.description,
        x.family,
        x.characteristics.join(","),
        "" + x.canteen.name,
        "" + x.priceHt,
        "" + x.hasAttachment,
      ])
  ...
  mounted() {
    ...
    const self = this
    setTimeout(() => {
      self.pets.push(["Snoopy", "Dog"])
      self.pets.push(["Nemo", "Fish"])
      self.pets.push(["Tigger", "Cat"])
    }, 2000)
