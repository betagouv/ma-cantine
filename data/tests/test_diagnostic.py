...
class DiagnosticModelSaveTest(TransactionTestCase):
    ...
    def test_year_validation(self):
        ...
        # on save
        for VALUE_OK_ON_SAVE in [None, -2000, 0, 1991, "2023", 2024, 2100]:
        ...
        for VALUE_NOT_OK_ON_FULL_CLEAN in [2024]:
    ...
    def test_diagnostic_type_validation(self):
        ...
        for VALUE_NOT_OK_ON_FULL_CLEAN in [None, "", 123, "invalid"]:
            with self.subTest(diagnostic_type=VALUE_NOT_OK_ON_FULL_CLEAN):
                ...
                self.assertRaises((ValueError, ValidationError), diagnostic.full_clean)
    ...
    def test_diagnostic_valeur_totale_validation(self):
        ...
        # on save
        diagnostic = DiagnosticFactory(**VALID_DIAGNOSTIC_WITHOUT_VALEUR_TOTALE)
        diagnostic.valeur_totale = None
        diagnostic.save()
        for VALEUR_TOTALE_VALUE_OK_ON_SAVE in [-1000, 0, 1000, Decimal("1234.56")]:
...
class Diagnostic2026ModelSaveTest(TransactionTestCase):
    def test_diagnostic_2026_without_type_is_not_populated_with_zero(self):
    ...
    def test_diagnostic_2026_simple_required_fields_are_populated_with_zero(self):
    ...
    def test_diagnostic_2026_complete_required_fields_are_populated_with_zero(self):
        diagnostic = Diagnostic.objects.create(
            year=2026, diagnostic_type=Diagnostic.DiagnosticType.COMPLETE, valeur_viandes_volailles_bio=10
        )
        self.assertIsNone(diagnostic.valeur_totale)  # not populated (not covered)
