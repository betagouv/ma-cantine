...
def validate_valeur_totale(instance):
    ...
    if instance.valeur_totale is not None:
        if instance.valeur_totale <= 0:
            ...
            for label in get_teledeclaration_field_groups(instance.year, "APPRO_LABELS_ALL"):
...
def validate_valeur_famille(instance):
    ...
    """
    errors = {}
    for family in get_teledeclaration_field_groups(instance.year, "APPRO_FAMILIES"):
    ...
    if instance.valeur_viandes_volailles is not None:
        ...
        if (
            ...
            utils_utils.add_validation_error(
                errors,
                "valeur_viandes_volailles",
                f"La valeur totale (HT) viandes et volailles fraiches ou surgelées, Origine France, {instance.valeur_viandes_volailles_france}, est plus que la valeur totale (HT) viandes et volailles, {instance.valeur_viandes_volailles}",
            )
    return errors
    ...
    if instance.valeur_produits_de_la_mer is not None:
        ...
        if (
            ...
            utils_utils.add_validation_error(
                errors,
                "valeur_produits_de_la_mer",
                f"La valeur totale (HT) poissons et produits aquatiques, Origine France, {instance.valeur_produits_de_la_mer_france}, est plus que la valeur totale (HT) poissons et produits aquatiques, {instance.valeur_produits_de_la_mer}",
            )
    return errors
