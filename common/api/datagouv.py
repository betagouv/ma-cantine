...
def map_pat_list_to_communes_insee_code():
    ...
    try:
        ...
        for pat in pat_list:
            pat_insee_code_list = [insee_code for insee_code in pat["communes_code_insee"].split(",") if insee_code]
            for pat_insee_code in pat_insee_code_list:
                if pat_insee_code not in pat_mapping.keys():
                    pat_mapping[pat_insee_code] = []
                pat_mapping[pat_insee_code].append(
                    {
                        "pat": pat["id"],
        ...
        for insee_code in pat_mapping.keys():
            if len(pat_mapping[insee_code]) > 2:
                print(f"Pat mapping : {insee_code} : {pat_mapping[insee_code]}")
    except requests.HTTPError as e:
        logger.info(e)
    return pat_mapping
