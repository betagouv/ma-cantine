...
def fill_missing_geolocation_libelle_data():
    ...
    epci_list = []
    with open("macantine/geo/epcis.csv", "r") as epci_file:
        reader = csv.DictReader(epci_file)
        epci_list = list(reader)
    ...
    for i, canteen in enumerate(candidate_canteens):
        ...
        if canteen.epci and not canteen.epci_lib:
            canteen.epci_lib = next((epci["nom"] for epci in epci_list if epci["code"] == canteen.epci), None)
            if canteen.epci_lib:
                update = True
        if update:
            canteen.save()
