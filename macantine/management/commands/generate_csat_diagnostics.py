...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Step 0 : Delete all diagnostics already generated by the script
        Diagnostic.objects.filter(generated_from_central_kitchen_diagnostic=True, year=year).delete()
        ...
        for diag in diagnostics_cc:
            ...
            nbre_satellites = len(satellites) if satellites else 0
            ...
            if production_type == Canteen.ProductionType.CENTRAL:
                if nbre_satellites == 0:
                    logger.error(
                        f"Task fail for diagnostic {diag.id} : Empty satellites, cannot distribute appro values"
                    )
                    continue
                ...
                create_new_diag_from_cc(
                    diag, diag.canteen_snapshot["id"], fields, updated_appro_fields, central_serving=True
                )
            else:
                logger.error(
