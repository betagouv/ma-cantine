...
export const hasDiagnosticApproData = (diagnostic) => {
  return diagnostic ? diagnostic.isFilled : false
}
      ...
      if (value) {
        if (field.endsWith("Bio")) {
          ...
          if (field.endsWith("LabelRouge") || field.endsWith("AocaopIgpStg")) {
            ...
            externalitesPerformanceTotal += value
          } else {
            egalimAutresTotal += value
          }
        }
      }
    })
...
export const hasSatelliteInconsistency = (canteen) => {
  ...
  if (canteen.satellitesMissingDataCount > 0) return true
  return false
}
