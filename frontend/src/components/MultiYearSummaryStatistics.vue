...
export default {
  ...
  computed: {
    seriesData() {
      return {
        ...
        total: this.completedDiagnostics.map((diagnostic) => diagnostic.valeurTotale),
      }
    },
    series() {
      return [
  ...
  methods: {
    ...
    otherPercentage(diagnostic) {
      return 100 - this.bioPercentage(diagnostic) - this.sustainablePercentage(diagnostic)
    },
  },
}
