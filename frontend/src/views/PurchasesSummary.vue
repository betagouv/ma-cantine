<template>
  <div class="text-left">
    ...
    <div>
      ...
      <div v-if="summary">
        <v-row class="mb-2">
          <v-col cols="12" sm="6" md="4" v-if="summary.valeurTotale">
            <v-card class="fill-height text-center py-4 d-flex flex-column justify-center" outlined>
              <p class="ma-0">
                <span class="grey--text text-h5 font-weight-black text--darken-2 mr-1">
                  {{ toCurrency(summary.valeurTotale) }}
                </span>
                <span class="caption">
                  total HT
                </span>
          ...
          <v-col col="8" sm="6" v-if="summary.valeurTotale && (showMealCountField || !mealCost)">
...
export default {
  ...
  computed: {
    ...
    bioPercent() {
      return this.summary && getPercentage(this.summary.valeurBio, this.summary.valeurTotale)
    },
    sustainablePercent() {
      return this.summary && getPercentage(getSustainableTotal(this.summary), this.summary.valeurTotale)
    },
    yearlyMealCount() {
      ...
      return [
        {
          ...
          data: Object.values(this.yearlySummary).map((s) => s.valeurTotale),
        },
      ]
    },
    years() {
