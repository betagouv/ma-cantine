...
export default {
  ...
  computed: {
    ...
    hasError() {
      return [this.totalMeatPoultryError, this.meatPoultryError].some((x) => !!x)
    },
    errorMessages() {
      return [this.totalMeatPoultryErrorMessage, this.meatPoultryErrorMessage].filter((x) => !!x)
    },
  },
  methods: {
    updatePayload() {
