...
const defaultFlatDiagnostics = [defaultDiagnostic(2019), defaultDiagnostic(2020)]
...
async function getDiagnostics() {
  ...
  if (jwt) {
    ...
    const localDiagnostics = (getLocalDiagnostics() || defaultFlatDiagnostics).sort(
      (earlierDiag, laterDiag) => laterDiag.year - earlierDiag.year
    )
...
function getLocalDiagnostics() {
  ...
  if (!hasFlatDiagnostics && hasStructuredDiagnostics) {
    ...
    if (Object.keys(diagnostics["information-des-usagers"]).indexOf("communicationSupport") !== -1) {
      diagnostics["information-des-usagers"].communicationSupports =
        diagnostics["information-des-usagers"].communicationSupport
...
function getDiagnosticsForDashboard() {
  if (
    ...
    !!localStorage.getItem(LOCAL_FLAT_KEY) ||
