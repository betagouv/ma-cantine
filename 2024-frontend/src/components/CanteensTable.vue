...
const getDropdownLinks = (canteen) => {
  ...
  if (canteen.productionType === "groupe") {
    links.push({
      to: { name: "GestionnaireCantineGroupeListe", params: { canteenUrlComponent: canteenUrlComponent } },
      ...
      <template #cell="{ colKey, cell }">
        <template v-if="colKey === 'name'">
          ...
          <p v-if="cell.satellitesCountSentence" class="fr-text-title--blue-france fr-mb-0 fr-text--sm">
            ...
            <router-link :to="{ name: 'GestionnaireCantineGroupeListe', params: { canteenUrlComponent: cell.url } }">
