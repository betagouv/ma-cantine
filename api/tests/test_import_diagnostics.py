...
class TestImportDiagnosticsAPI(APITestCase):
    ...
    def test_diagnostics_created(self, mock):
        ...
        self.assertEqual(diagnostic.value_total, 1000)
    ...
    def test_success_complete_diagnostic_import(self, mock):
        ...
        self.assertEqual(finished_diag.value_total, 10500)
        ...
        self.assertEqual(finished_diag.label_sum("hve"), 110)
        self.assertEqual(finished_diag.label_sum("peche_durable"), 120)
        self.assertEqual(finished_diag.label_sum("rup"), 130)
        self.assertEqual(finished_diag.label_sum("commerce_equitable"), 140)
        self.assertEqual(finished_diag.label_sum("fermier"), 150)
        self.assertEqual(finished_diag.label_sum("externalites"), 160)
        self.assertEqual(finished_diag.label_sum("performance"), 170)
        self.assertEqual(finished_diag.label_sum("non_egalim"), 180)
        self.assertEqual(finished_diag.label_sum("france"), 190)
        self.assertEqual(finished_diag.label_sum("short_distribution"), 200)
        self.assertEqual(finished_diag.label_sum("local"), 210)
        self.assertEqual(finished_diag.family_sum("viandes_volailles"), 110)
        self.assertEqual(finished_diag.family_sum("produits_de_la_mer"), 110)
        self.assertEqual(finished_diag.family_sum("fruits_et_legumes"), 110)
        self.assertEqual(finished_diag.family_sum("charcuterie"), 110)
        self.assertEqual(finished_diag.family_sum("produits_laitiers"), 110)
        self.assertEqual(finished_diag.family_sum("boulangerie"), 110)
        ...
        self.assertEqual(unfinished_diag.value_total, 30300)  # picked a field at random to smoke test
    ...
    def test_complete_diagnostic_error_collection(self, mock):
        ...
        self.assertEqual(
            errors[2]["message"],
            "Champ 'Poissons, produits de la mer et de l'aquaculture, Bio' : La valeur « lol » doit être vide ou un nombre décimal.",
        )
        self.assertEqual(
            errors[3]["message"],
    ...
    def test_success_cuisine_centrale_complete_import(self, mock):
        ...
        self.assertEqual(unfinished_diag.diagnostic_type, Diagnostic.DiagnosticType.COMPLETE)
        self.assertEqual(unfinished_diag.value_total, 30300)
        self.assertEqual(unfinished_diag.value_viandes_volailles, None)
        self.assertEqual(unfinished_diag.value_produits_de_la_mer, 10)
    ...
    def test_success_cuisine_centrale_simple_import(self, mock):
        ...
        self.assertEqual(cc1_diag.value_total, 10500)
        ...
        self.assertEqual(cc2_diag.value_total, 30300)
    ...
    def test_update_existing_diagnostic(self, mock):
        ...
        diagnostic = DiagnosticFactory(canteen=canteen, year=2021, value_total=1, value_bio=0.2)
    ...
    def test_update_diagnostic_conditional_on_teledeclaration_status(self, mock):
        ...
        with freeze_time(date_in_2022_teledeclaration_campaign):
            ...
            diagnostic.refresh_from_db()
            self.assertEqual(diagnostic.value_total, 1)
