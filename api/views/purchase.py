    ...
    def get(self, request, *args, **kwargs):
        ...
        if is_canteen_manager:
            data["last_purchase_date"] = (
                Purchase.objects.for_canteen_and_year(canteen, year).only("date").latest("date").date
            )
...
def canteen_summary_for_year(canteen, year):
    purchases = Purchase.objects.for_canteen_and_year(canteen, year).only(
        "id", "family", "characteristics", "price_ht"
    )
    data = {"year": year}
