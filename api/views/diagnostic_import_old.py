...
class ImportCompleteDiagnosticsView(ImportDiagnosticsView):
    ...
    def _validate_diagnostic(self, row):
        ...
        try:
            values_dict["valeur_totale"] = Decimal(row[value_idx].strip().replace(",", "."))
        except InvalidOperation:
            raise ValidationError({"valeur_totale": "Ce champ ne peut pas être vide."})
        for value in ["valeur_viandes_volailles", "valeur_produits_de_la_mer"] + Diagnostic.APPRO_FIELDS_FOR_IMPORT:
            try:
